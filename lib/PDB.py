    ...
    def link(self, tabname):
        ...
        try:
            if self.key.startswith("fk"):
                ...
                return "ALTER TABLE %s ADD FOREIGN KEY (%s) REFERENCES %s;" % (
                    tabname, self.name, ref)
            else:
                return None
        except(AttributeError):
            return None
...
class db(object):
    ...
    def __init__(self, name, verbose=False, test=False):
        ...
        if test:
            ...
            self.db = MySQLdb.connect(
                HOSTIP, USER, PASSWD, DBNAME, autocommit=True)
    ...
    def populate(self):
        ...
        for t in self.tabs.keys():
            q = self[t].link()
            for _q in q.split(';'):
                # a wrapper to deal with _mysql's inability to parse multiple
    ...
    def has_record(self, tabname, key, col=None):
        ...
        if col is None:
            q = "SELECT EXISTS(SELECT 1 FROM %s WHERE %s='%s');" % (
                tabname, self[tabname].pk, key)
        else:
            q = "SELECT EXISTS(SELECT 1 FROM %s WHERE %s='%s');" % (
                tabname, col, key)
        if self.verbose:
    ...
    def get(self, target, tab, col, val):
        ...
        if isinstance(col, list):
            constraints = [" %s=%s " % (c, self.format_values(
                tab, {c: v})[0]) for (c, v) in zip(col, val)]
    ...
    def update(self, target_col, target_val, tab, col, val):
        ...
        q = """UPDATE %s SET %s=%s WHERE %s=%s;""" % (
            tab, target_col, target_val, col, val)
        if self.verbose:
            print q
        self.db.query(q)
...
class StillDB(db):
    ...
    def list_observations(limit=1e5):
        ...
        # basefile not in (select basefile from history where
        # status='COMPLETE');""")
...
#        else: return False
    ...
    def get_neighbors(basefile):
        ...
        and host={host}';""".format(
            ...
            filename=filename,
            host=host)
        # this line uncompresses the 2d result to a 1d list
        neighbors = [b[0] for b in self.query(q)]
        return neighbors
    ...
    def begin_task(infile, outfile, operation, pid):
        ...
        if not self.has_record('files', infile):
            ...
            return 1
        self.update('last_modified', "NOW()", 'files', 'filename',
                    infile)  # update the files table entry
        ...
        # get the infile basefile, note that we already checked that it exists
        # in the files table.
        histcols['basefile'] = self.get('basefile', 'files', 'filename', infile)[0][
            0]  # get basefile from files where filename=infile
    ...
    def add_file(filename):
        ...
        filecols['basefile'] = self.query(
            """select basefile from observations where JD={JD} and pol={pol};""")
    ...
    def kill_task(filename):
        ...
        """
        tasks = self.get('PID', 'history', [
                         'filename', 'exit_status'], [filename, 'NULL'])
        host = filename.split(':')[0]
        for task in tasks:
            # note I am not 100% on this line here
            os.Popen("ssh {host} 'kill -9 {pid}'".format(host=host, pid=pid))
