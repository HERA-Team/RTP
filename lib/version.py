...
def construct_version_info():
    ...
    try:
        git_origin = subprocess.check_output(['git', '-C', rtp_dir, 'config',
                                              '--get', 'remote.origin.url'],
                                             stderr=subprocess.STDOUT).strip()
        git_hash = subprocess.check_output(['git', '-C', rtp_dir, 'rev-parse', 'HEAD'],
                                           stderr=subprocess.STDOUT).strip()
        git_description = subprocess.check_output(['git', '-C', rtp_dir,
                                                   'describe', '--dirty', '--tag', '--always']).strip()
        git_branch = subprocess.check_output(['git', '-C', rtp_dir, 'rev-parse',
                                              '--abbrev-ref', 'HEAD'],
                                             stderr=subprocess.STDOUT).strip()
        git_version = subprocess.check_output(['git', '-C', rtp_dir, 'describe',
                                               '--tags', '--abbrev=0']).strip()
...
# String to add to history of any files written with this version of RTP
rtp_version_str = ('rtp version: ' + version + '.')
if git_hash is not '':
    rtp_version_str += ('  Git origin: ' + git_origin +
                        '.  Git hash: ' + git_hash +
