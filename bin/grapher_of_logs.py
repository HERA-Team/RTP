...
def create_array_from_csv(csvfilename):
    ...
    with open(csvfilename) as csv_log_file:
        ...
        for logentry in csv_reader:
            time_diff = dateparser.parse(
                logentry['end_time']) - dateparser.parse(logentry['start_time'])
            if logentry['end_time']:
                time_diff_min = round(time_diff.seconds / 60.0, 1)
                log_status_array.append({'obsid': logentry['obsnum'], 'stage': logentry[
                                        'stage'], 'start_time': logentry['start_time'], 'length_of_time': time_diff_min})
...
def main():
    ...
    # Put the legend in the best possible place
    plt.rcParams['legend.loc'] = 'best'
    plt.figure(1)

    log_status_array = create_array_from_csv(csvfilename)

    sorted_list_for_time = sorted(
        log_status_array, key=lambda k: k['start_time'])
    init_start_time = sorted_list_for_time[0]['start_time']
    global_end_time = ((dateparser.parse(sorted_list_for_time[len(
        sorted_list_for_time) - 1]['start_time']) - dateparser.parse(init_start_time)).seconds) / 60
    global_end_time = global_end_time + \
        (sorted_list_for_time[len(sorted_list_for_time) - 1]['length_of_time'])
    print("End time : %s") % (global_end_time)

    sorted_list = sorted(log_status_array, key=lambda k: (
        k['stage'], k['start_time']))
        ...
        if logentry['stage'] not in list_of_processed_stages:
            ...
            for log_entry_by_stage in sorted_list:
                if log_entry_by_stage['stage'] == logentry['stage']:
                    ...
                    time_since_beginning = dateparser.parse(
                        log_entry_by_stage['start_time']) - dateparser.parse(init_start_time)
                    date_array_x.append((time_since_beginning.seconds) / 60)
            plt.plot(date_array_x, time_array_y, '-o', label=logentry[
                     'stage'] + " - Avg: " + str(round(numpy.mean(time_array_y), 1)) + " m")
            avg_time_for_run = avg_time_for_run + \
                round(numpy.mean(time_array_y), 1)
