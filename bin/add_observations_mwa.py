...
def ingest_addtional_opsids(sg):
    ...
    try:
        pgconn = psycopg2.connect(database=sg.dbname, user=sg.dbuser,
                                  host=sg.dbhost, port=sg.dbport, password=sg.dbpasswd)
    except:
        print("I am unable to connect to the database")
        exit(1)
    print("Probably connected")
# cur_dict = pgconn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
# # Get a sql cursor that supports returning data as a dict
    ...
    # as well as files that are in the mwa_qc db and have thus already been
    # processed
    ...
    rows = cur.fetchall()
    # Lets just make sure we trim out all the extra obs id's we get from each
    # having multiple files associated with it
