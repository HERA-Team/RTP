...
def main(args):
    ...
    o.add_option(
        '--connection', help='the name of the Librarian connection to use (as in ~/.hl_client.cfg)')
    o.add_option('--config_file',
                 help='RTP configuration file default=RTP/etc/still.cfg', default='etc/still.cfg')
    ...
    def augment_record(r):
        return {
            # NOTE: this is actually free text
            'obsnum': os.path.basename(r['store_path']),
            'date': r['date'],
            'date_type': 'julian',
            'pol': r['pol'],
            'host': r['host'],
    ...
    for pol in pols:
        soi = sorted((oi for oi in obsinfos if oi[
                     'pol'] == pol), key=lambda oi: oi['date'])
