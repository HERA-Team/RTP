...
o.add_option(
    ...
    dest='time',
    default='all',
    help='Select which time sample to plot. Options are: "all" (default), "<time1 #>_<time2 #>" (a range of times to plot), or "<time1 #>,<time2 #>" (a list of times to plot). If "all" or a range are selected, a 2-d image will be plotted. If a list of times is selected an xy plot will be generated.')
...
def gen_times(timeopt, uv, coords, decimate):
    if timeopt == 'all':
        ...
        if len(timeopt[0]) != 1:
            ...
            def time_selector(t, cnt):
                if coords == 'index':
                    return cnt in timeopt
                for opt in timeopt:
                    if (t >= opt) and (t < opt + inttime):
                        return True
                return False
...
if opts.delay:
    if freqs.size == freqs[-1] - freqs[0] + 1:
        ...
        raise ValueError(
            'Channels must be contiguous to do delay transform (chan=%s)' % (opts.chan))
...
def sort_func(a, b):
    ...
    if bi > ai or (bi == ai and bj > aj) or (
            bi == ai and bj == aj and pb < pa):
        return -1
    return 1
...
for cnt, bl in enumerate(bls):
    ...
    if opts.fringe:
        d = d.filled(0)
        w = a.dsp.gen_window(d.shape[0], window=opts.window)
        w.shape += (1,)
        ...
        if not opts.clean is None:
            for chan in range(d.shape[1]):
                if gain[chan] == 0:
                    continue
                d[:, chan], info = a.deconv.clean(
                    d[:, chan], ker[:, chan], tol=opts.clean)
    ...
    if is_chan_range and is_time_range:
        if opts.fringe:
            if opts.time_axis == 'index':
                drates = n.fft.fftfreq(
                    len(plot_t['cnt']), 1. / len(plot_t['cnt']))
                step = drates[1] - drates[0]
                ylabel = 'Delay-Rate (bins)'
            else:
                drates = n.fft.fftfreq(
                    len(plot_t['cnt']), inttime) * 1e3  # mHz
                step = drates[1] - drates[0]
            ...
            if opts.time_axis == 'index':
                step = 1.
                t1, t2 = plot_t['cnt'][0] - 0.5 * \
                    step, plot_t['cnt'][-1] + 0.5 * step
                ylabel = 'Time (integrations)'
            elif opts.time_axis == 'lst':
                step = plot_t['lst'][1] - plot_t['lst'][0]
                t1, t2 = (plot_t['lst'][0] - 0.5 * step) * 12 / \
                    n.pi, (plot_t['lst'][-1] + 0.5 * step) * 12 / n.pi
                ylabel = 'Local Sideral time (hrs)'
            else:
                step = plot_t['jd'][1] - plot_t['jd'][0]
                t1, t2 = plot_t['jd'][0] - 0.5 * \
                    step, plot_t['jd'][-1] + 0.5 * step
                ylabel = 'Time (Julian Date)'
        if opts.delay:
            if opts.chan_axis == 'index':
                step = 1
                c1, c2 = len(chans) / 2 - len(chans) - 0.5 * \
                    step, len(chans) / 2 - 0.5 * step
                xlabel = 'Delay (bins)'
            else:
        ...
        if not opts.xlim is None:
            p.xlim(*opts.xlim)
        if not opts.ylim is None:
            # Reverse b/c otherwise ylim flips origin for unknown reasons
            p.ylim(opts.ylim[1], opts.ylim[0])
        ...
        if opts.fringe:
            ...
            if opts.time_axis == 'index':
                ...
                raise ValueError('Unrecognized time axis type.')
        if opts.chan_axis == 'index':
            label += '#%d'
        else:
            chans = freqs
        ...
        raise ValueError('Either time or chan needs to be a range.')

    if opts.pretty:  # formatting tick marks and subplots_adjust
        p.xlabel(xlabel, fontsize=10)
        if (cnt % m1):
            p.yticks([])
            p.ylabel('')  # puts y-ticks and y-labels only on left side
        if ((m2 - 1) * m1 > (cnt % nsub + (m2 * m1 - nsub))):
            p.xticks([])  # puts x-ticks only on bottom
        p.subplots_adjust(wspace=0, hspace=0, left=0.05,
                          right=0.98, bottom=0.07, top=0.98)
    ...
    if not opts.share:
        ...
        if opts.pretty:  # formatting title
            p.text(plot_chans[len(plot_chans) / 8], dmin +
                   (dmax - dmin) / 4, title, fontsize=6)
        else:
            p.title(title)

if not opts.nolegend and (not is_time_range or not is_chan_range):
    if opts.pretty:
        p.legend(loc=4, prop={'size': 6})  # formatting legend
    else:
        p.legend(loc='best')

# Save to a file or pop up a window
if opts.out_file != '':
    p.savefig(opts.out_file)
else:
    def click(event):
        print [event.key]
        if event.key == 'm':
