...
def normalize_data(datadict):
    ...
    for key in datadict.keys():
        d[key] = datadict[key] / \
            n.where(n.abs(datadict[key]) == 0., 1., n.abs(datadict[key]))
    return d
...
times, data, flags = arp.get_dict_of_uv_data(
    args, bl_string, opts.pol, verbose=True)
datapack = {}  # not necessarily xx data inside
for (i, j) in data.keys():
    datapack[(i, j)] = data[(i, j)][opts.pol]
# XXX less hacky than previous hardcode, but always safe?
nfreq = datapack[datapack.keys()[0]].shape[1]
...
# Save solutions
if len(args) == 1:
    save_gains(sols, fqs, opts.pol, filename=args[
               0], ubls=ubls, ex_ants=ex_ants)
else:
    save_gains(sols, fqs, opts.pol, ubls=ubls, ex_ants=ex_ants)
