...
o.add_option('--verbose', '-v', dest='verb',
             action='store_true', help='print intermediate info')
o.add_option('--wfall', '-W', dest='save_wfall', action='store_true',
             help='Save occupancy waterfall .png file?')
o.add_option('--focc', '-F', dest='save_freq', action='store_true',
             help='Save frequency occupancy .png file? Same naming convention as default .npz file.')
o.add_option('--npz', '-N', dest='npz', default=None,
             help='Name of output .npz (neglecting the ".npz" ending) with flag grid and percentage info. Default will be <JD>_<ant>_<pol>_RFI.npz')
o.add_option('--wimg', dest='wimg', default=None,
             help='Name of waterfall .png file. Default will be same naming convention as default .npz file.')
o.add_option('--fimg', dest='fimg', default=None,
             help='Name of waterfall .png file. Default will be <first file>_f.png .')
o.add_option('--show', '-S', dest='show',
...
pcnt_t, pcnt_f = [], []
if opts.verb:
    print 'Calculating percentage occupations'
...
if opts.npz is not None:
    npzname = opts.npz
else:
    npzname = '%s_%s_%s_RFI.npz' % (jd, opts.ant, opts.pol)

if opts.verb:
    print 'Writing data to %s' % npzname
...
if not opts.show and not opts.save_wfall and not opts.save_freq:
    sys.exit()
...
if opts.save_freq or opts.show:
    if opts.verb:
        print 'Plotting frequency-occupancy plot'
    pylab.step(fqs, pcnt_f, where='mid')
    pylab.fill_between(fqs, 0, pcnt_f, color='blue', alpha=0.3)
    pylab.xlabel('Frequency [MHz]')
    pylab.ylabel('Occupancy [%]')
    if len(args) == 1:
        pylab.suptitle(file2jd(args[0]), size=15)
    else:
        pylab.suptitle('%s - %s' %
                       (file2jd(args[0]), file2jd(args[len(args) - 1])), size=15)

    # pylab.savefig('%s_%s_%s_F.png'%(jd,opts.ant,opts.pol)) #hard to plug
    # into RTP
    if not opts.fimg is None:
        pylab.savefig(opts.fimg)
    else:
        # zen.2451234.12345.xx.HH.uvcR_f.png
        pylab.savefig('%s_f.png' % args[0])
...
# Plotting waterfall
if opts.verb:
    print 'Plotting occupancy waterfall'
# Set-up plot format
left, width = 0.1, 0.65
...
axImshow.imshow(flg_arr, aspect='auto', interpolation='nearest',
                cmap='binary', extent=[100., 200., t_arr[len(t_arr) - 1], t_arr[0]])
...
if opts.save_wfall:
    if not opts.wimg is None:
        pylab.savefig(opts.wimg)
    else:
        pylab.savefig('%s_%s_%s_RFI.png' % (jd, opts.ant, opts.pol))
if opts.show:
    pylab.show()
pylab.close()
