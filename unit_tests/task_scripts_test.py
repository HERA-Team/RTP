...
class TestRawScripts(unittest.TestCase):
    ...
    def test_script_chain(self):
        ...
        for b in basefiles:
            subprocess.call(
                ['do_UV.sh', b, 'localhost:%s/data/%s' % (os.getcwd(), b)])
        ...
        subprocess.call(['do_ACQUIRE_NEIGHBORS.sh',
                         'localhost:/tmp/test1.uvcR', 'localhost:/tmp/test3.uvcR'])
        ...
        subprocess.call(['do_UVCRRE.sh', 'test1.uvcR',
                         'test2.uvcRR', 'test3.uvcR'])
